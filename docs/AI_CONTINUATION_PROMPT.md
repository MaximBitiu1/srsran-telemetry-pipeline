# AI Continuation Prompt — srsRAN 5G NR jBPF Telemetry + ZMQ Channel Broker

> **Give this entire document to a new AI session.** It contains everything needed
...
4. **ZMQ Channel Broker** — custom C program injecting AWGN + Rayleigh flat fading into IQ samples between gNB and UE
...
|------|-------|-------------|
| `~/Desktop/zmq_channel_broker.c` | 477 | AWGN + Rician fading IQ broker (C) |
| `~/Desktop/zmq_channel_broker` | — | Compiled C binary |
| `~/Desktop/srsran_channel_broker.py` | ~950 | GNU Radio Python broker (superset of C: EPA/EVA/ETU, CFO, drops, scenarios, GUI) |
| `~/Desktop/launch_mac_telemetry.sh` | ~520 | Pipeline launcher (flags: `--snr N`, `--fading`, `--k-factor N`, `--grc`, `--gui`, `--profile P`, `--cfo N`, `--drop-prob N`, `--scenario S`, `--no-broker`, `--no-ue`, `--no-traffic`, `--no-grafana`) |
| `~/Desktop/stop_mac_telemetry.sh` | 105 | Pipeline teardown in reverse order |
...
| `~/Desktop/AI_CONTINUATION_PROMPT.md` | — | This handoff document |
...
└──────────────────────────────────────┘
    │
    ▼
srsUE (RX:2000, TX:2001) ──► iperf3 (10 Mbps UDP uplink)
```
...
1. **Run with downlink traffic** (iperf3 server→UE) to populate DL-side plots
...
13. **Add DL iperf3 mode** to launch script (reverse direction traffic to populate DL panels)
