...
uint64_t jbpf_main(void* state)
{
    ...
    out = (harq_stats *)jbpf_map_lookup_elem(&stats_map_dl_harq, &zero_index);
    if (!out) return JBPF_CODELET_FAILURE;
    asm volatile("" : "+r"(out));

    if (new_val) {
        MAC_HARQ_STATS_INIT_DL(out->stats[safe_ind], ctx->cell_id, ctx->rnti, ctx->du_ue_index);
        out->stats[safe_ind].max_nof_harq_retxs = harq_info.h.max_nof_harq_retxs;
        out->stats[safe_ind].mcs_table = harq_info.h.mcs_table;
    }
    ...
    if (reinterpret_cast<const uint8_t*>(&harq_info) + sizeof(jbpf_mac_sched_harq_ctx_info_dl) <= reinterpret_cast<const uint8_t*>(ctx->data_end)) {    
        ...
        // cons_retx
        STATS_UPDATE(out->stats[safe_ind].cons_retx, harq_info.h.nof_retxs); 

        // mcs
        STATS_UPDATE(out->stats[safe_ind].mcs, harq_info.h.mcs); 

        // perHarqTypeStats — use fixed indices to avoid double variable offset in BPF
        {
            uint32_t ht = harq_info.h.harq_type % JBPF_HARQ_EVENT_NUM;
            ...
            if (ht == 0) {
                ...
                TRAFFIC_STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[0].tbs_bytes, harq_info.h.tbs_bytes);
                STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[0].cqi, harq_info.cqi);
            } else if (ht == 1) {
                out->stats[safe_ind].perHarqTypeStats[1].count++;
                TRAFFIC_STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[1].tbs_bytes, harq_info.h.tbs_bytes);
                STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[1].cqi, harq_info.cqi);
            } else {
                out->stats[safe_ind].perHarqTypeStats[2].count++;
                TRAFFIC_STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[2].tbs_bytes, harq_info.h.tbs_bytes);
                STATS_UPDATE(out->stats[safe_ind].perHarqTypeStats[2].cqi, harq_info.cqi);
            }
        }
    }
