...
uint64_t jbpf_main(void *state)
{
    ...
    // Force the BPF loader to register ul_harq_hash in relocations
    // (required for cross-codelet map linking with stats & ue_deletion codelets)
    { uint32_t _dk = 0; (void)jbpf_map_lookup_elem(&ul_harq_hash, &_dk); }
    ...
    void* c = jbpf_map_lookup_elem(&stats_map_ul_harq, &zero_index);
    if (!c)
        return JBPF_CODELET_FAILURE;
    ...
    if (*not_empty_ul_harq_stats && *last_timestamp_ul_harq < timestamp32)
    {
        ...
        // JBPF_HASHMAP_CLEAR removed: verifier rejects complex loop
        
        // Reset the info
        // NOTE: this is not thread safe, but we don't care here
        // The worst case we can overwrite someone else writing
        // jbpf_map_clear removed: verifier rejects it for large maps
