...
if $START_UE; then
  ...
  # Wait for RRC connection + PDU session
  info "Waiting for UE to attach (up to 30s)..."
  UE_READY=false
  for i in $(seq 1 30); do
    if grep -q "PDU Session" "$LOG_UE" 2>/dev/null; then
      UE_READY=true
      break
    fi
    sleep 1
  ...
  if $UE_READY; then
    # Wait for TUN interface to actually come up (up to 15s after PDU session)
    TUN_UP=false
